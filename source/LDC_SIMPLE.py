...
def main():
    ...
    pS_u = np.zeros(shape = (nx+1,ny+1)) # not used
    pd = np.zeros(shape = (nx+1,ny+1)) # not used
    ...
    while (error>errcon and niter<maxiter):
        ...
        for i in range(1,nx-1):
            for j in range(1,ny): 
                u_star_e = (u_star[i-1,j] + u_star[i,j])/2
                u_star_w = (u_star[i+1,j] + u_star[i,j])/2
                v_star_n = (v_star[i,j-1] + v_star[i+1,j-1])/2
                v_star_s = (v_star[i,j] + v_star[i+1,j])/2
                ...
                #print([F_w,F_e,F_n,F_s])

                del_F = F_e - F_w + F_n - F_s
                ...
                uS_u[i,j] = (p_star[i+1,j]-p_star[i,j]-alpha_p*ua_P[i,j]*u_old[i,j])*dy*t
        ...
        for i in range(1,nx-1):
            for j in range(1,ny): 
                u_star[i,j] = (ua_W[i,j]*u_star[i+1,j] + ua_E[i,j]*u_star[i-1,j] + ua_N[i,j]*u_star[i,j-1] + ua_S[i,j]*u_star[i,j+1] + \
                            uS_u[i,j])/ua_P[i,j]
                
                
                
        # Boundary Conditions
        #u_star[:,-1] = 0 #-u_star[:,-2] # South
        #u_star[-1:] = 0 ###-u_star[-2,:] # West 
        #u_star[0,:] = 0 ###-u_star[1,:] # East
        #u_star[:,0] = U # North
        ...
        for i in range(1,nx):
            for j in range(1,ny-1):
                u_star_e = (u_star[i-1,j] + u_star[i-1,j+1])/2
                u_star_w = (u_star[i,j] + u_star[i,j+1])/2
                v_star_n = (v_star[i,j-1] + v_star[i,j])/2
                v_star_s = (v_star[i,j+1] + v_star[i,j])/2
                ...
                vS_u[i,j] = (p_star[i,j+1]-p_star[i,j] - alpha_p * va_P[i, j] * v_old[i, j])*dy*t # Su = (P_S - P_N)*A_s
        ...
        for i in range(1,nx):
            for j in range(1,ny-1): 
                v_star[i,j] = (va_W[i,j]*v_star[i+1,j] + va_E[i,j]*v_star[i-1,j] + va_N[i,j]*v_star[i,j-1] + va_S[i,j]*v_star[i,j+1] + \
                            vS_u[i,j])/va_P[i,j]
                
                
        # Boundary Conditions
        #v_star[:,-1] = 0 #####-v_star[:,-2] # South
        #v_star[-1:] = 0  #-v_star[-2,:] # West 
        #v_star[0,:] = 0 #-v_star[1,:] # East
        #v_star[:,0] = 0 ### North
        ...
        p_new = (1-alpha_p)*p_old + alpha_p*p
        u_new = (1-alpha_u)*u_old + alpha_u*u
        v_new = (1-alpha_v)*v_old + alpha_v*v
